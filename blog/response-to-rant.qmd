...
asked the maintainer for the mute messages argument to be added it to the wrapper functions such as `KL()` it likely would have been. And indeed, the current development version has an option to set it persistently, when calls to distance are being made, as seen here
...
(and the `textmodel_seededlda()` function is just a small wrapper around `textmodel_lda()`),
