...
If you discover an issue with a package (that you use regularly), why wouldn't you try to reach out to the author? Or instead of using stargazer consider one of the more modern alternatives,
which are more maintained, such as [modelsummary](https://cran.r-project.org/web/packages/modelsummary/index.html).
...
See e.g. the newly released package [mori](https://cran.r-project.org/web/packages/mori/index.html), which seems to be a very promising method for reducing memory when doing parallel processing in R (released after the author wrote the rant).
...
Another option is the `glue` package, where you can then write the above
like this:
...
But anyway, for checking objects I can recommend the [checkmate](https://cran.r-project.org/web/packages/checkmate/index.html) package, which
...
Just use e.g. [data.table](https://cran.r-project.org/web/packages/data.table/index.html) (or it's tidy-variant [tidytable](https://cran.r-project.org/web/packages/tidytable/index.html)) and
...
That doesn't align with what the author said above at all, and to me feels like the author is being disingenuous. And very likely if the author
asked the maintainer for the mute messages argument to be added it to the wrapper functions such as `philentropy::KL()` it likely would have been. And indeed, the current development version has an option to set it persistently, when calls to `philentropy::distance()` are being made, as seen here
...
Now the author complains about the package [seededlda](https://cran.r-project.org/web/packages/seededlda/index.html), and that
...
This took me 2 minutes to find. And once again, if you have an issue with a package, consider making an issue/PR on GitHub, instead of writing a rant about it. The maintainer of the package is likely to be very responsive and helpful, and will probably (try to) fix any issues you have.
...
What? Does the author also think that packages like [Rcpp](https://cran.r-project.org/web/packages/Rcpp/index.html) to call C++, [JuliaCall](https://cran.r-project.org/web/packages/JuliaCall/index.html) to call Julia code, [rJava](https://cran.r-project.org/web/packages/rJava/index.html) to call Java code, [rextendr](https://cran.r-project.org/web/packages/rextendr/index.html) to call Rust code, and so on, also have no reason to exist? There are many valid reasons to want to call code from another language without switching completely to said language. So yes,
