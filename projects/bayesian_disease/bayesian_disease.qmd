...
Now we can solve for the parameters numerically.
...
fit_sir_negbin <- stan(file = file_path, 
                       data = data_sir,
                       seed = 1405,)
```
...
- Mean $1/4$ and $P(\gamma\leq 0.4)=0.9$. This gives us the prior $$\gamma \sim \mathrm{Normal}^+(0.2435308, \; 0.12).$$  
