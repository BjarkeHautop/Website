...
image: "bayesian-disease/image.jpg"
date: 07/18/2024
...
file_path <- file.path(
  "bayesian-disease/stan_model.stan"
)
```
...
We applied a Bayesian approach to model disease outbreaks using the SIR framework, estimating key parameters like the basic reproduction number, $R_0$, while accounting for uncertainty. Model validation, including posterior predictive checks and sensitivity analyses, confirmed the robustness of our results.

The Bayesian SIR model provides a practical tool for understanding disease dynamics and supporting public health decisions. Future work could extend it to more complex models and other infectious diseases.
